...
<!-- tdd-code-fingerprint: 92fd22ccb4cffa2ea0d4e77618167cf8f7472408 -->
...
[`src/core/currency/currency-selectors.ts`](https://github.com/EdgeApp/edge-core-js/blob/9f28c9ffcbd7f647e0832ca482d480dc5203274c/src/core/currency/currency-selectors.ts)
```ts
/**
...
[`src/core/currency/wallet/currency-wallet-api.ts`](https://github.com/EdgeApp/edge-core-js/blob/9f28c9ffcbd7f647e0832ca482d480dc5203274c/src/core/currency/wallet/currency-wallet-api.ts)
```ts
...
| `enabledTokenIds`, `currencyConfig` accessors | `resyncBlockchain`, `dumpData`, staking actions |
| `stakingStatus`, served from the cache because the GUI subtracts locked stake from the balance, so an empty stake over a cached balance reports more spendable than the wallet holds | |
...
The pre-engine serve gate is `opts.forceIndex == null && cachedAddresses.length > 0 && engineFailure == null && engine == null`, and a serve also bumps the wallet to the front of the startup queue, since a caller on an address screen wants this engine early. Every chain serves the cache and no caller opts in; the `allowCached` option and the `hasStableAddresses` hint that once gated this are gone ([decision 7.9](#79-the-provisional-receive-affordance-reverted)). Correctness moved from a gate to a reconcile: a query answered from the cache is recorded, re-asked of the engine in the background once it loads, and `rememberAddresses` emits `addressChanged` when the engine's first answer differs, so every consumer re-queries. An engine that confirms the cached answer emits nothing. Review turned up three details the mechanism depends on:
...
- The reconcile's engine wait is not an app-visible error at teardown. A logout destroys the pixie under the pending wait and a wallet deletion drops it from the state, so an unguarded rejection would reach `onError` and surface a shutdown or wallet-missing error after the user has left the account. Both are swallowed, because the correction they were going to deliver is moot once the wallet is gone (test case 44).
...
In `account-cache.test.ts`, for the write generations ([section 6.7](#67-followup-the-post-review-followups-landed)):

31. A rename from another device applies after a local rename, because the load that the sync starts began after the local write.

In `engine-scheduler.test.ts` and `wallet-cache.test.ts`, for the reviewer-bot findings in the same section:

32. Logout releases in-flight startup slots for the next login: with one slot held open inside engine creation, a logout followed by a new login reaches engine creation while the old session's gate is still held.
33. Logout during engine startup swallows a late startup failure: an engine creation that fails after the account logged out emits no error.
34. A token toggle made before the token file loads reaches the cache once the load lands, even though the merged list is unchanged.
35. A name load that began before a local rename is dropped: with the fake disk's read gate parking the load on the value it already read, a rename in that window wins and the stale load does not apply.
36. A cache write left in flight by a logout lands before the next login writes: with the fake disk's write gate parking session A's write, session B's first write waits behind it and the two slots hold consecutive generations.
37. A wallet whose files have not loaded keeps its cache entry across a write: the saver's retention is keyed on the account's wallet list, not on the sparse `walletStates`.
38. A wallet keeps reaching the cache after a resync: the engine clear resets `tokenFileLoaded`, but the saver gates on a sticky `tokenFileEverLoaded`, so a rename made after the resync is written.
39. Adding a custom token to a legacy plugin (tools without `getTokenId`) while the engines are still queued waits for one of that plugin's engines instead of failing with "a wallet must exist".
40. That wait skips a wallet whose engine already failed: with the plugin's first wallet failed and its second still queued, the add waits for the second instead of rejecting with the first one's error.
41. A fallback read that outlives its logout does not seed the next session: with the loader's gate parking session A's pixie on the seed it read for an unarchived wallet, session B renames that wallet, and the parked read resumes into a destroyed pixie without touching B's name.
42. A wallet created after boot seeds from the cache when unarchived: the saver's write put it in the memo, so with the engine gated the unarchive emits the wallet from its entry, the same as a wallet the boot seeded.
43. Startup loads that outlive a logout do not touch the next session: with session A's fiat-file load parked past its read and the account logged out, a fiat code pushed from another device survives the parked load's release in the next session, because the startup block's dispatches go quiet at destroy.
44. A reconcile that outlives its logout stays silent: with the engine gated, a cached-address query parks its background reconcile on the engine wait, and logging out rejects that wait without reaching the context's `error` event.
45. An engine callback fired after its logout leaves the next session alone: the first session's callbacks are kept, and the balance one is called once the next login's wallet is up under the same id; that wallet keeps its own balance.
46. A staking query that outlives its logout reports no error: the engine's `getStakingStatus` is parked past the logout, and its rejection reaches nobody.
47. A token list survives an unreadable token file: with both token files corrupted on disk, the startup load keeps the cache-seeded list, and the next cache write still carries it.
...
### 6.6 Followup: first human review (landed)

The first full human review of both branches produced six core findings and one gui finding. Four core findings and the gui finding changed the design; two were answered and left as they stood.

| Finding | Shipped as |
| --- | --- |
| Engine created with empty `userSettings`, because nothing ordered the deferred `reloadPluginSettings` ahead of it | Engine startup waits for the whole deferred account load (custom tokens, plugin settings, builtin tokens) before `makeCurrencyEngine`, rejecting on `ACCOUNT_LOAD_FAILED` ([decision 7.7](#77-per-concern-load-gates-not-one-loadallfiles-promise)) |
| `getFallbackDisklets` in the wallet API duplicated `makeStorageWalletApi`'s own pre-repo fallback | Deleted; both getters delegate to `storageWalletApi` |
| A wallet outside the bulk seed re-read both cache slots, ~2 x 105 KiB each | The account pixie keeps the parsed boot file for the session; a miss goes straight to the per-wallet files ([section 4.1.2](#412-load-path)) |
| `stakingStatus` was uncached while the classification table called it engine-gated | Cached, and the table corrected ([section 4.1.4](#414-makecurrencywalletapi-changes)) |
| A cached wallet whose engine failed rendered as a healthy row, since the loading row that carried the error is unreachable once the wallet object exists | The shared wallet row shows the engine error in the overlay slot that paused and disabled wallets already use, so the row and its balances stay |

Two findings were answered rather than changed, both because the proposed fix costs more than the behavior it corrects:

- **A batch of bumps runs in reverse of the order it was asked for.** Making the insert preserve call order requires walking past entries already marked bumped, and those marks accumulate from incidental startup bumps, so a live user bump lands behind them. At batch sizes within the concurrency of 8 the ordering is unobservable.
- **`nameDirty` and its siblings clear only on a `fromFile` load.** The writers dispatch after the file write is durable, so the flag exists to beat a load that started before that write. Clearing it in the save path would hand that in-flight load the win. Removing the dependency on the sync server's echo needs a per-file load generation, which is deferred.

The review also moved one invariant. Engine startup is now a reader that gates on the deferred account load, where the per-concern gates previously covered only writers.

### 6.7 Followup: the post-review followups (landed)

The three follow-ups the post-review runs recorded, each shipped or dispositioned.

| Follow-up | Shipped as |
| --- | --- |
| Whole-value wallet files kept a dirty flag that cleared only on a `fromFile` load, so correctness rested on the sync server echoing the local write | Write generations ([section 4.1.5](#415-save-path)): a load applies only when the generation it began under is still current. Test case 31 |
...
- **Chosen: per-concern gates.** Engine startup is the one reader that gates too: it waits for the deferred account load (custom tokens, plugin settings, builtin tokens) before `makeCurrencyEngine`, because an engine built with empty `userSettings` opens connections before a `networkPrivacy` setting can apply. Each write awaits only its dependency. Wallet-file writes await the wallet repo, token file writes await `tokenFileLoaded`, settings writes await `pluginSettingsLoaded`, wallet-state writes await the account repo. A rename never waits on custom tokens.
