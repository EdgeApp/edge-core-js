...
export const walletPixie: TamePixie<CurrencyWalletProps> = combinePixies({
  ...
  engine(input: CurrencyWalletInput) {
    ...
    // Work that outlives a logout must not touch the next session:
    // its pixie under the same wallet id would take this one's stale
    // reads as its own, and the cache saver would then persist them.
    // Every dispatch of the startup block and of the engine callbacks
    // goes through this, so anything that lands after `destroy` is
    // dropped:
    ...
    async function update(): Promise<unknown> {
      ...
      try {
        ...
        const engine = await plugin.makeCurrencyEngine(publicWalletInfo, {
          // The engine keeps firing between `destroy` and the moment
          // `killEngine` finishes, and the next login already has a
        ...
        // The wallet can also be deleted (or the account logged out)
        // during the awaits above, which the slot-acquisition check
        // cannot see. By now `engineStarted`'s destroy has already run
        // and found no engine to kill, so publishing this one would
        // leave it alive with nothing left to tear it down:
        if (destroyed) {
          await engine.killEngine().catch(() => {})
          return
        }
        ...
        if (engine.getStakingStatus != null) {
          await engine.getStakingStatus().then(
            ...
            // A logout rejects this query for a wallet the user has
            // already left, which is teardown, not an error the app
            // can act on (the catch below drops the same case):
            error => {
              if (destroyed) return
