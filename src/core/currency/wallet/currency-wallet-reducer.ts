...
>({
  ...
  enabledTokenIds: sortStringsReducer(
    (state = initialTokenIds, action, next, prev): string[] => {
      if (action.type === 'CURRENCY_WALLET_LOADED_TOKEN_FILE') {
        // No readable file, so keep the cache-seeded list and any
        // unsaved toggles already applied to it:
        const { enabledTokenIds } = action.payload
        if (enabledTokenIds == null) return state
