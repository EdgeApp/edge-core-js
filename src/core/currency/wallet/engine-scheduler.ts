...
function makeEngineScheduler(): EngineScheduler {
  ...
  return {
    ...
    bump(walletId) {
      // Search from the back. A wallet destroyed while queued leaves
      // its entry behind (nothing rejects the promise it is waiting
      // on), so if that id is recreated its live entry is the newer
      // one and promoting the dead one would leave the live pixie at
      // the back of the line:
      let index = -1
      for (let i = queue.length - 1; i >= 0; --i) {
        if (queue[i].walletId === walletId) {
          index = i
          break
        }
      }
