...
interface ChangeServerCallbacks {
  ...
  handlePluginConnect: (pluginId: string) => void
  handlePluginDisconnect: (pluginId: string) => void
}
...
export function connectChangeServer(
  ...
  const codec = changeProtocol.makeClientCodec({
    ...
    localMethods: {
      ...
      pluginConnect(params) {
        callbacks.handlePluginConnect(params.pluginId)
      },
      pluginDisconnect(params) {
        callbacks.handlePluginDisconnect(params.pluginId)
      }
    }
  })
