...
export interface FakePluginTestConfig {
  ...
  /**
   * If set, spread over the engine's `getFreshAddress` answer, so a
   * test can make a freshly loaded engine disagree with the address
   * its predecessor cached.
   */
  freshAddressPatch?: Partial<EdgeFreshAddress>
   ...
   */
  onEngineKill?: (walletId: string) => void

  /**
   * If set, receives the callbacks handed to each engine, so tests can
   * fire one after its session is gone.
   */
  onEngineCallbacks?: (
    walletId: string,
  ...
  /**
   * If set, `getStakingStatus` returns this promise instead of the
   * engine's own status, so a test can hold or reject that query.
   */
  stakingStatusHook?: () => Promise<EdgeStakingStatus>
}
...
class FakeCurrencyEngine implements EdgeCurrencyEngine {
  ...
  async killEngine(): Promise<void> {
    this.running = false
    if (fakePluginTestConfig.onEngineKill != null) {
      fakePluginTestConfig.onEngineKill(this.walletId)
    }
  }
  ...
  async getStakingStatus(): Promise<EdgeStakingStatus> {
    if (fakePluginTestConfig.stakingStatusHook != null) {
      return await fakePluginTestConfig.stakingStatusHook()
    }
    return {
  ...
  async getFreshAddress(
    ...
    return {
      ...
      ...fakePluginTestConfig.freshAddressPatch
    }
  }
